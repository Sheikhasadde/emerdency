...
function my_insert($data){
	...
	if ($telAdvice == 2){
		...
		$sql = "INSERT INTO Appointment (`practiceDentist_id`, `start_date`, `end_date`, `text`, dis_access) VALUES ({$pdId},'{$start}', '{$end}', '{$text}', {$disAccess});";
	}
	$conn->sql->query($sql);
	$data->success();
}
...
$conn->render_sql("SELECT id, start_date, end_date, text, dis_access AS disAccess, practiceDentist_id AS pdId, type AS telAdvice FROM Appointment", "id", "start_date, end_date, text, disAccess, pdId, telAdvice");